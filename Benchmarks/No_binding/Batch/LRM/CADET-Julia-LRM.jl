...
c_analytical = CSV.read((joinpath(@__DIR__,"LRM_analytical.csv")),DataFrame)
...
inlets, outlets, columns, switches, solverOptions = model_setup(nCell[2],polyDeg[1], 0, true)
...
plot!(c_analytical[:,"C0"])

# Evaluate the convergence using the evaluate_convergence function 
evaluate_IDA(c_analytical, nComp, nCell, polyDeg, 1, "LRM", @__DIR__, IDA(linear_solver =:KLU))
