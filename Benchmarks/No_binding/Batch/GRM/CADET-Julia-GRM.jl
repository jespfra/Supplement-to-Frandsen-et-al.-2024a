...
include(joinpath(@__DIR__, fill("..", 2)..., "evaluate_IDA.jl"))
...
c_analytical = CSV.read((joinpath(@__DIR__,"Semi-analytical_GRM.csv")),DataFrame)
...
function model_setup(nCells, polyDeg, polyDegPore, exactInt, analJac=false)
	...
	model["root"]["input"]["model"]["unit_001"]["adsorption"]["LIN_KA"] = [0]
	model["root"]["input"]["model"]["unit_001"]["adsorption"]["LIN_KD"] = [0]
...
inlets, outlets, columns, switches, solverOptions = model_setup(nCell[2],polyDeg[1], polyDegPore[1], 0, true)
...
evaluate_IDA(c_analytical, nComp, nCell, polyDeg, polyDegPore, "GRM", @__DIR__, IDA(linear_solver =:KLU))
